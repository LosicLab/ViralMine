...
# HBV Genotypes for each patient. Can use a "bootstrapping" contig-size normalization method, or
	...
	for gt in gts:
		if gt != '':
			if re.findall("[1-9]",gt) != []:
				gt_scores[gt[0]] = float(gt.split(' ')[1])
			else:
				gt_scores[gt[0]] = 0
		else:
			pass
...
for pat in patients:
	...
	tot_nscores = {"A":0,"B":0,"C":0,"D":0,"E":0,"F":0,"G":0}
...
with open('./output_table.tsv','w') as out:
	out.write("Patient\tA\tB\tC\tD\tE\tF\tG\n")
	for pat in patients:
		vals = []
